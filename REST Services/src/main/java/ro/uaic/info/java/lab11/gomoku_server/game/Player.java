...
import ro.uaic.info.java.lab11.gomoku_server.services.GameCallService;
import ro.uaic.info.java.lab11.gomoku_server.services.PlayerCallService;
...
//                representGame();
    ...
    void uploadGame() throws JSONException {
        GameCallService gameCallService = new GameCallService();
        String response = gameCallService.addGame(game);
        // get game id from response
        String[] words = response.split(" ");
        int gameId = Integer.parseInt(words[1]);

        PlayerCallService playerCallService = new PlayerCallService();
        playerCallService.addPlayer(this, gameId);
    }
}
